...
def prepare_data(data_mat, data_path_src, save_dir, dataset_type='train', generate_npy=False, debug_mode=False):
  ...
  for ex_idx, ex in enumerate(data_mat[0]):
      ...
      try:
        ...
        if not os.path.exists(image_path):
          ...
          continue
            ...
            gray = cv2.resize(cv2.cvtColor(face, cv2.COLOR_BGR2GRAY), (224,224))
            face_cv = cv2.resize(face, (224, 224))
            detector = dlib.get_frontal_face_detector()
            predictor = dlib.shape_predictor(u"C:\\Users\\Sathwika\\OneDrive\\Documents\\ucsd\\spis\\emotic_scripts\\shape_predictor_68_face_landmarks.dat") 
            dets = detector(gray, 1)
            for det in dets:
                shape = predictor(gray, det)
                for landmark_index in range(0, 68):
                    x = shape.part(landmark_index).x
                    y = shape.part(landmark_index).y
                    cv2.circle(face_cv, (x, y), 2, (0, 255, 0), -1)
    ...
    if debug_mode == True and idx >= 104:
      ...
      cv2.imwrite(os.path.join(save_dir, 'face1.png'), face_arr[-1])
      break
  print (to_break, path_not_exist)

  if generate_npy == True: 
    face_arr = np.array(face_arr)
    print (len(data_set), face_arr.shape)
    np.save(os.path.join(save_dir,'%s_face_arr.npy' %(dataset_type)), face_arr)
